// drive_pengembalian.js (multi-upload)
...
let tokenClient;
window.accessToken = "";
window.uploadedPhotosPengembalian = []; // array untuk menyimpan semua foto

// ===== Tombol & Elemen =====
const loginBtn = document.getElementById("loginBtn");
const uploadBtn = document.getElementById("uploadBtn");
const captureBtn = document.getElementById("captureBtn");
const FotoPengembalianInput = document.getElementById("FotoPengembalian");
...
captureBtn.addEventListener("click", () => {
  ...
  FotoPengembalianInput.value = dataUrl;
...
uploadBtn.addEventListener("click", async () => {
  if (!FotoPengembalianInput.value) {
  ...
  try {
    const result = await uploadToDrive(FotoPengembalianInput.value, window.accessToken);
    console.log(result);
    ...
    window.uploadedPhotosPengembalian.push({
      fotoIdPengembalian: result.id,
      fileLink: `https://drive.google.com/uc?id=${result.id}`
    });

    // Simpan ke dataset input hidden untuk Peminjaman.js
    FotoPengembalianInput.dataset.fotoIdPengembalian = window.uploadedPhotosPengembalian.map(f => f.fotoIdPengembalian).join(",");
    FotoPengembalianInput.dataset.fileLink = window.uploadedPhotosPengembalian.map(f => f.fileLink).join(",");

    // Reset foto untuk ambil lagi
    FotoPengembalianInput.value = "";
    previewImage.src = "";
    ...
    uploadDebug.textContent += "\n🔹 Total foto diarray: " + window.uploadedPhotosPengembalian.length;
